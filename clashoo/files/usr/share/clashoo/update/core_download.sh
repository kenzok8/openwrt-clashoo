...
if [ "$CORETYPE" = "4" ] || [ "$CORETYPE" = "5" ]; then
	...
	if [ -n "$OPENWRT_ARCH" ]; then
		...
		write_log "优先尝试 OpenWrt 包: ${ASSET}"
		if download_with_mirrors "$URL" /tmp/singbox-openwrt.ipk && install_singbox_openwrt_asset /tmp/singbox-openwrt.ipk; then
			installed=1
		else
			write_log "OpenWrt 包安装失败，尝试 tar 包兜底"
		fi
	fi
	...
	if [ "$installed" -ne 1 ]; then
		for ASSET in "sing-box-${VER}-linux-${SINGBOX_ARCH}-musl.tar.gz" "sing-box-${VER}-linux-${SINGBOX_ARCH}.tar.gz"; do
			...
			if download_with_mirrors "$URL" /tmp/singbox.tar.gz && install_singbox_tar_asset /tmp/singbox.tar.gz; then
				installed=1
				break
			fi
		done
